(() => {
  ...
  const injectStyles = () => {
    ...
    style.textContent = `
      .admin-panel{position:fixed;bottom:24px;left:24px;width:320px;max-width:92vw;background:rgba(26,45,55,0.94);color:#e8f4ff;border:1px solid rgba(6,182,212,0.4);border-radius:16px;padding:18px;box-shadow:0 18px 38px rgba(0,0,0,0.4);z-index:9999;font-family:'Inter','Segoe UI',sans-serif;backdrop-filter:blur(12px);}
      ...
      .admin-panel__input{flex:1 1 140px;padding:10px 12px;border-radius:10px;border:1px solid rgba(6,182,212,0.5);background:rgba(12,20,30,0.85);color:#e8f4ff;font-size:14px;}
      .admin-panel__btn{padding:10px 14px;border-radius:10px;border:none;background:linear-gradient(135deg,#06b6d4,#0891b2);color:#ecfeff;font-weight:600;cursor:pointer;transition:transform .15s ease, box-shadow .15s ease;}
      .admin-panel__btn:hover{transform:translateY(-1px);box-shadow:0 12px 20px rgba(6,182,212,0.35);}
      .admin-panel__btn--ghost{background:rgba(6,182,212,0.16);color:#e8f4ff;}
      .admin-panel__btn--ghost:hover{box-shadow:0 12px 20px rgba(6,182,212,0.25);}
      .admin-panel__hint{font-size:13px;color:#99c5d6;margin:0;}
      .admin-panel__status{font-size:13px;color:#22d3ee;margin:0;}
      .admin-panel__section--meta{font-size:13px;color:#99c5d6;}
      .admin-panel__section--meta button{width:100%;}
      .admin-wa-target{outline:1px dashed rgba(6,182,212,0.6);outline-offset:4px;position:relative;}
      .admin-wa-target::after{content:'WA';position:absolute;top:-10px;right:-10px;font-size:11px;background:rgba(6,182,212,0.85);color:#ecfeff;padding:2px 6px;border-radius:999px;}
